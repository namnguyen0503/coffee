        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
          <li class="nav-item menu-open">
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item"><a href="javascript:void(0)" class="nav-link" id="link-cashflow" onclick="chuyenCheDo('cashflow')"><i class="fas fa-money-bill-wave nav-icon"></i><p>Kiểm soát tiền ca</p></a></li>
      ...
      <div class="container-fluid">
        ...
        <div class="row mb-2 align-items-center">
          <div class="col-sm-6 d-flex align-items-center">
              ...
              <span id="btn-group-menu" class="mode-section">
                <button class="btn btn-success btn-sm shadow-sm" data-toggle="modal" data-target="#modalAddItem">
                    <i class="fas fa-plus-circle"></i> Thêm món mới
                </button>
                <button class="btn btn-outline-success btn-sm shadow-sm ml-2" onclick="xuatMenuExcel()">
                    <i class="fas fa-file-excel"></i> Xuất Excel
                </button>
              </span>

              <span id="btn-group-order" class="mode-section" style="display:none;">
                <div class="d-flex align-items-center bg-white p-1 rounded shadow-sm border">
                    <span class="mr-2 text-muted ml-2"><i class="far fa-calendar-alt"></i></span>
                    <label class="mr-1 mb-0 font-weight-normal text-sm">Từ:</label>
                    <input type="date" id="order_start" class="form-control form-control-sm mr-3 border-0" style="background:#f4f6f9; width: 135px;">
                    <label class="mr-1 mb-0 font-weight-normal text-sm">Đến:</label>
                    <input type="date" id="order_end" class="form-control form-control-sm mr-3 border-0" style="background:#f4f6f9; width: 135px;">
                    <button class="btn btn-primary btn-sm mr-2" onclick="taiNoiDung()"><i class="fas fa-filter"></i> Xem</button>
                    <button class="btn btn-success btn-sm" onclick="xuatDonHangExcel()"><i class="fas fa-file-excel"></i> Xuất Excel</button>
                </div>
              </span>
              
              <span id="btn-group-user" class="mode-section">
                  <button class="btn btn-primary btn-sm shadow-sm" data-toggle="modal" data-target="#modalAddUser"><i class="fas fa-user-plus"></i> Thêm nhân viên</button>
                  <button class="btn btn-outline-primary btn-sm shadow-sm ml-2" onclick="xuatUserExcel()"><i class="fas fa-file-excel"></i> Xuất Excel</button>
              </span>
              
              <span id="btn-group-voucher" class="mode-section">
                  <button class="btn btn-info btn-sm shadow-sm" data-toggle="modal" data-target="#modalAddVoucher"><i class="fas fa-ticket-alt"></i> Tạo mã</button>
                  <button class="btn btn-outline-info btn-sm shadow-sm ml-2" onclick="xuatVoucherExcel()"><i class="fas fa-file-excel"></i> Xuất Excel</button>
              </span>
              ...
              <span id="btn-group-cashflow" class="mode-section" style="display:none;">
                <div class="d-flex align-items-center bg-white p-1 rounded shadow-sm border">
                    <span class="mr-2 text-muted ml-2"><i class="fas fa-search-dollar"></i></span>
                    <label class="mr-1 mb-0 font-weight-normal text-sm">Từ:</label>
                    <input type="date" id="cash_start" class="form-control form-control-sm mr-2 border-0" style="background:#f4f6f9; width: 135px;">
                    <label class="mr-1 mb-0 font-weight-normal text-sm">Đến:</label>
                    <input type="date" id="cash_end" class="form-control form-control-sm mr-2 border-0" style="background:#f4f6f9; width: 135px;">
                    
                    <div class="custom-control custom-switch mr-3 ml-2">
                        <input type="checkbox" class="custom-control-input" id="checkDiffOnly">
                        <label class="custom-control-label text-sm" for="checkDiffOnly">Chỉ hiện lệch</label>
                    </div>

                    <button class="btn btn-primary btn-sm mr-2" onclick="taiCashFlow()"><i class="fas fa-filter"></i> Xem</button>
                    <button class="btn btn-success btn-sm" onclick="xuatCashFlowExcel()"><i class="fas fa-file-excel"></i> Excel</button>
                </div>
              </span>
          </div>
        ...
        <div id="hienthi-cashflow" class="mode-section" style="display:none">
                    ...
                    <table class="table table-head-fixed text-nowrap table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>Nhân viên</th>
                                <th>Thời gian</th>
                                <th class="text-right text-muted">Chốt ca trước</th>
                                <th class="text-right font-weight-bold">Vào ca này (Start)</th>
                                <th class="text-center">Chênh lệch</th>
                                <th class="text-right">Doanh thu</th>
                                <th class="text-right">Tiền chốt ca</th>
                                <th>Ghi chú</th>
                            </tr>
                        </thead>
        ...
        <div id="hienthi-schedule" class="mode-section">
            ...
            <div class="card card-info card-outline">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-list"></i> Danh sách chi tiết ca làm việc</h3>
                </div>
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3 bg-light p-2 rounded border">
                        <label class="mb-0 mr-2">Từ ngày:</label>
                        <input type="date" id="list_start" class="form-control form-control-sm mr-3" style="width: 140px;">
                        
                        <label class="mb-0 mr-2">Đến ngày:</label>
                        <input type="date" id="list_end" class="form-control form-control-sm mr-3" style="width: 140px;">
                        
                        <button class="btn btn-primary btn-sm mr-2" onclick="taiDanhSachLichList()">
                            <i class="fas fa-filter"></i> Lọc danh sách
                        </button>
                        
                        <button class="btn btn-success btn-sm" onclick="xuatExcelLichList()">
                            <i class="fas fa-file-excel"></i> Xuất Excel
                        </button>
                    </div>
...
<script>
    ...
    // --- HÀM XỬ LÝ NGÀY THÁNG ĐỊA PHƯƠNG ---
    ...
    $(document).ready(function(){
        ...
        var firstDayStr = getLocalDateString(firstDay);

        // Tính ngày đầu tuần
        var day = today.getDay(); 
        var currentDay = (day === 0) ? 7 : day; 
        var monday = new Date(today);
        monday.setDate(today.getDate() - (currentDay - 1));
        currentWeekStart = getLocalDateString(monday);

        // Set mặc định cho các ô chọn ngày
        $('#report_end, #order_end, #cash_end').val(todayStr);
        $('#report_start, #order_start, #cash_start').val(firstDayStr);
        
        $('#list_end').val(todayStr);
        $('#list_start').val(firstDayStr);
    ...
    window.chuyenCheDo = function(mode) {
        ...
        $('.mode-section').hide();
        $('#hienthi-sanpham, #hienthi-baocao, #hienthi-schedule, #hienthi-warehouse, #hienthi-cashflow').hide();
        $('#searchForm').show();
        ...
        if(mode == 'menu') {
            ...
            $('#link-order').addClass('active'); $('#page-title').text('Quản lý Đơn hàng'); 
            $('#btn-group-order').show(); 
            $('#hienthi-sanpham').show(); taiNoiDung();
            ...
            $('#link-schedule').addClass('active'); $('#page-title').text('Lịch làm việc'); 
            $('#searchForm').hide(); 
            $('#btn-group-schedule').show(); 
            $('#hienthi-schedule').show();   
            taiLichLamViec();
            taiDanhSachLichList();
        } else if(mode == 'stock') {
            $('#link-stock').addClass('active'); $('#page-title').text('Cảnh báo & Cấu hình kho'); 
            $('#searchForm').hide(); 
            $('#hienthi-warehouse').show(); loadStockData();
        
        // --- CHẾ ĐỘ MỚI: CASHFLOW ---
        } else if(mode == 'cashflow') {
            $('#link-cashflow').addClass('active'); $('#page-title').text('Kiểm soát tiền ca');
            $('#searchForm').hide();
            $('#btn-group-cashflow').show();
            $('#hienthi-cashflow').show();
            taiCashFlow(); // Gọi hàm tải dữ liệu mới
        }
    };
    ...
    // --- LOGIC KIỂM SOÁT TIỀN CA (MỚI) ---
    window.taiCashFlow = function() {
        var start = $('#cash_start').val();
        var end = $('#cash_end').val();
        var diffOnly = $('#checkDiffOnly').is(':checked'); // Lấy trạng thái checkbox

        $('#cashflow-body').html('<tr><td colspan="9" class="text-center">Đang tải dữ liệu...</td></tr>');

        $.ajax({
            url: 'api/get_cash_flow.php',
            type: 'GET',
            data: {start: start, end: end, diff_only: diffOnly},
            dataType: 'json',
                ...
                if(res.data && res.data.length > 0) {
                    res.data.forEach(function(row) {
                        html += `<tr>
                            ...
                            <td>
                                <div><small>BĐ:</small> ${row.start_fmt}</div>
                                <div><small>KT:</small> ${row.end_fmt}</div>
                            </td>
                            <td class="text-right text-muted">${row.prev_end_cash_display}</td>
                            <td class="text-right font-weight-bold" style="font-size:1.1em">${row.start_cash_fmt}</td>
                            <td class="text-center">${row.diff_html}</td>
                            <td class="text-right text-success">${row.total_sales_fmt}</td>
                            <td class="text-right font-weight-bold">${row.end_cash_fmt}</td>
                            <td><small>${row.note || ''}</small></td>
                        </tr>`;
                    });
                } else {
                    html = '<tr><td colspan="9" class="text-center">Không tìm thấy ca làm việc nào.</td></tr>';
                }
    ...
    window.xuatCashFlowExcel = function() {
        var start = $('#cash_start').val();
        var end = $('#cash_end').val();
        var diffOnly = $('#checkDiffOnly').is(':checked');
        window.location.href = 'api/export_cash_flow.php?start=' + start + '&end=' + end + '&diff_only=' + diffOnly;
    }
    ...
    window.taiDanhSachLichList = function() {
        var start = $('#list_start').val();
        var end = $('#list_end').val();
        
        $('#schedule-list-body').html('<tr><td colspan="4" class="text-center">Đang tải dữ liệu...</td></tr>');

        $.ajax({
            url: 'api/get_schedule_list.php', type: 'GET', data: {start: start, end: end}, dataType: 'json',
            success: function(res) {
                var html = '';
                if(res.data && res.data.length > 0) {
                    res.data.forEach(function(item) {
                        html += `<tr><td>${item.date_fmt}</td><td class="font-weight-bold">${item.day_name}</td><td>${item.shift_txt}</td><td class="text-primary font-weight-bold">${item.fullname}</td></tr>`;
                    });
                } else {
                    html = '<tr><td colspan="4" class="text-center">Không tìm thấy ca làm việc.</td></tr>';
                }
                $('#schedule-list-body').html(html);
            }
        });
    }

    window.xuatExcelLichList = function() {
        var start = $('#list_start').val();
        var end = $('#list_end').val();
        window.location.href = 'api/export_schedule_list.php?start=' + start + '&end=' + end;
    }

    // --- CÁC HÀM KHÁC ---
    window.xuatMenuExcel = function() { window.location.href = 'api/export_items.php'; }
    window.xuatDonHangExcel = function() { window.location.href = 'api/export_orders.php?start='+$('#order_start').val()+'&end='+$('#order_end').val(); }
    window.xuatVoucherExcel = function() { window.location.href = 'api/export_vouchers.php'; }
    window.xuatUserExcel = function() { window.location.href = 'api/export_users.php'; }
    ...
    function luuPhanCa(e, forceUnlock = false) {
        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.status == 'success') { $('#modalAddSchedule').modal('hide'); alert(res.message); taiLichLamViec(); taiDanhSachLichList(); } 
    ...
    function xoaCa(id) { 
        if(confirm("Xóa?")) 
            $.ajax({ 
                url: 'api/delete_schedule.php', type: 'POST', data: {id: id}, 
                success: function(){ taiLichLamViec(); taiDanhSachLichList(); } 
            }); 
    }
    function formatDate(d) { return d.getDate()+'/'+(d.getMonth()+1); }
    window.taiNoiDung = function() {
        if(currentMode == 'report' || currentMode == 'schedule' || currentMode == 'stock' || currentMode == 'cashflow') return;
    ...
    $(document).on('click', '.btn-edit', function(e){ e.preventDefault(); var id = $(this).data('id'); $('#edit_id').val(id); $('#edit_name').val($(this).data('name')); $('#edit_price').val($(this).data('price').toString().replace(/\./g, '')); $('#edit_category').val($(this).data('category')); $('#edit_status').val($(this).data('status')); $('#preview_img').attr('src', $(this).data('img')); $('#recipe-container-edit').html(''); $.ajax({ url: 'api/get_recipe_detail.php', data: {product_id: id}, dataType: 'json', success: function(recipes) { if(recipes && recipes.length > 0) recipes.forEach(r => addIngredientRow('recipe-container-edit', r.ingredient_id, r.quantity_required)); else addIngredientRow('recipe-container-edit'); } }); $('#modalEdit').modal('show'); });
    ...
    function loadStockData() {
        $('#tbl-stock-body').html('<tr><td colspan="5" class="text-center">Đang tải...</td></tr>');
        $.ajax({
            url: 'api/get_ingredients.php', dataType: 'json',
            success: function(data) {
                var html = ''; var warningCount = 0;
                data.forEach(function(item) {
                    var qty = parseFloat(item.quantity); var min = parseFloat(item.min_quantity) || 0;
                    var isLow = qty <= min; if (isLow) warningCount++;
                    var statusHtml = isLow ? '<span class="badge badge-danger">Sắp hết</span>' : '<span class="badge badge-success">Ổn định</span>';
                    var rowStyle = isLow ? 'background-color: #fff3cd;' : '';
                    html += `<tr style="${rowStyle}"><td class="font-weight-bold">${item.name}</td><td class="text-center ${isLow?'text-danger':''}">${qty}</td><td class="text-center"><input type="number" class="form-control form-control-sm text-center input-min-qty mx-auto" style="max-width:100px" data-id="${item.id}" value="${min}" step="0.1"></td><td class="text-center">${item.unit}</td><td class="text-center">${statusHtml}</td></tr>`;
                });
                $('#tbl-stock-body').html(html);
                if (warningCount > 0) { $('#badge-stock-sidebar').text(warningCount).show(); } else { $('#badge-stock-sidebar').hide(); }
            }
        });
    }
    window.luuCauHinhKho = function() {
        var updates = []; $('.input-min-qty').each(function() { updates.push({ id: $(this).data('id'), min_quantity: $(this).val() }); });
        $.ajax({ url: 'api/update_min_stock.php', type: 'POST', data: { data: JSON.stringify(updates) }, dataType: 'json', success: function(res) { if(res.status == 'success') { alert('Cập nhật thành công!'); loadStockData(); } else { alert(res.message); } } });
    }
</script>
</body>
</html>